...
export default function TypingFunda() {
    ...
    function generateLessonPatterns() {
        ...
        // 🔹 Single-number 5 + next 5
        for (let i = 0; i < numberKeys.length - 1; i += 2) {
            const line = [
                ...Array(5).fill(numberKeys[i]),     // 5 of current
                ...Array(5).fill(numberKeys[i + 1])  // 5 of next
            ];
            practiceTexts.push(line.join(" "));
        }

        // 🔹 Two-number alternation (10 numbers)
        for (let i = 0; i < numberKeys.length - 1; i++) {
            const pair = [numberKeys[i], numberKeys[i + 1]];
            let line = [];
            for (let k = 0; k < 5; k++) line.push(...pair);
            practiceTexts.push(line.join(" "));
        }

        // 🔹 Sliding window of 10 numbers (wrap around)
        const mixed = numberKeys.concat(numberKeys); // wrap around
