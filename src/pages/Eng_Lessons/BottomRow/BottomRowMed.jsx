...
export default function HomeRow() {
    ...
    function generateLessonPatterns() {
        ...
        // 🔹 2-letter neighbor combinations (as sd df jk kl)
        for (let i = 0; i < bottomRowKeys.length - 1; i++) {
            let line = [];
            const pair = bottomRowKeys[i] + bottomRowKeys[i + 1];
            for (let j = 0; j < 5; j++) line.push(pair);
            practiceTexts.push(line.join(" "));
        }

        // 🔹 3-letter chunks (asd sdf jkl kl;)
        for (let i = 0; i < bottomRowKeys.length - 2; i++) {
            let line = [];
            const trio =
                bottomRowKeys[i] +
                bottomRowKeys[i + 1] +
                bottomRowKeys[i + 2];
            for (let j = 0; j < 3; j++) line.push(trio);
            practiceTexts.push(line.join(" "));
        }

        // 🔹 Mixed 2 + 3 letter patterns
        for (let i = 0; i < bottomRowKeys.length - 2; i++) {
            practiceTexts.push(
                `${bottomRowKeys[i]}${bottomRowKeys[i + 1]} ` +
                `${bottomRowKeys[i + 1]}${bottomRowKeys[i + 2]} ` +
                `${bottomRowKeys[i]}${bottomRowKeys[i + 1]}${bottomRowKeys[i + 2]}`
            );
        }

        // 🔹 Cross-hand 2–3 letter mixing (as jk, df kl)
        for (let i = 0; i < 4; i++) {
            practiceTexts.push(
                `${bottomRowKeys[i]}${bottomRowKeys[i + 1]} ` +
                `${bottomRowKeys[i + 4]}${bottomRowKeys[i + 5]} ` +
                `${bottomRowKeys[i]}${bottomRowKeys[i + 4]}${bottomRowKeys[i + 5]}`
            );
        }
