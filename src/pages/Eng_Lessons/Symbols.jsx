...
export default function TypingFunda() {
    ...
    function generateLessonPatterns() {
        ...
        // 🔹 Single-symbol 5 + next 5
        for (let i = 0; i < symbols.length - 1; i += 2) {
            const line = [
                ...Array(5).fill(symbols[i]),
                ...Array(5).fill(symbols[i + 1])
            ];
            practiceTexts.push(line.join(" "));
        }

        // 🔹 Two-symbol alternation (10 symbols)
        ...
        // 🔹 Sliding window of 10 symbols (wrap around)
        const mixed = symbols.concat(symbols);
