...
export default function MotherBoard() {
    ...
    function generateStoryLines() {
        const story = `
    A motherboard, also called the mainboard or system board, is the central circuit board of a computer. It is the main platform that connects all the hardware components and allows them to communicate with each other. Essentially, the motherboard acts as the backbone of the computer, linking the CPU, memory, storage devices, input/output devices, and expansion cards. Without a motherboard, a computer cannot function.

The motherboard contains a CPU socket, where the processor is installed. The processor, or central processing unit, performs all the calculations and processing tasks. Next to the CPU socket, there are RAM slots for installing memory modules, which temporarily store data for fast access during processing. The motherboard also has chipsets that control data flow between the CPU, memory, and other components.

Storage devices like hard disks or SSDs are connected to the motherboard via SATA, NVMe, or IDE ports. Expansion slots like PCIe and PCI slots allow the addition of graphics cards, sound cards, network cards, and other peripherals. Motherboards also include power connectors to supply electricity to the CPU and other components, and USB ports, audio jacks, and Ethernet ports for connecting external devices.

Modern motherboards come with integrated components, such as sound cards, network adapters, and sometimes even graphics processors, reducing the need for additional hardware. They also feature BIOS or UEFI firmware, which helps initialize hardware during startup and provides a basic interface for configuring the system.

Motherboards vary in size and form factor, including ATX, Micro-ATX, and Mini-ITX, to fit different types of computer cases. Choosing a compatible motherboard is essential, as it determines the type of CPU, memory, and expansion options a computer can support.

In summary, the motherboard is the heart of the computer, providing a platform for all components to work together efficiently. It ensures proper communication between the CPU, memory, storage, and peripheral devices. Without a motherboard, none of the computer’s parts could function as a unified system. A reliable motherboard is crucial for system stability, performance, and upgradability.
    `;
