...
export default function HomeRow() {
    ...
    function generateLessonPatterns() {
        ...
        for (let i = 0; i < homeRowKeys.length - 1; i++) {
            let line = [];
            const pair = homeRowKeys[i] + homeRowKeys[i + 1];
            for (let j = 0; j < 5; j++) line.push(pair);
            practiceTexts.push(line.join(" "));
        }

        // 🔹 3-letter chunks (asd sdf jkl kl;)
        for (let i = 0; i < homeRowKeys.length - 2; i++) {
            let line = [];
            const trio =
                homeRowKeys[i] +
                homeRowKeys[i + 1] +
        ...
        for (let i = 0; i < homeRowKeys.length - 2; i++) {
            practiceTexts.push(
                `${homeRowKeys[i]}${homeRowKeys[i + 1]} ` +
                `${homeRowKeys[i + 1]}${homeRowKeys[i + 2]} ` +
                `${homeRowKeys[i]}${homeRowKeys[i + 1]}${homeRowKeys[i + 2]}`
            );
        }
        ...
        for (let i = 0; i < 4; i++) {
            practiceTexts.push(
                `${homeRowKeys[i]}${homeRowKeys[i + 1]} ` +
                `${homeRowKeys[i + 4]}${homeRowKeys[i + 5]} ` +
                `${homeRowKeys[i]}${homeRowKeys[i + 4]}${homeRowKeys[i + 5]}`
            );
        }
