...
export default function HomeRow() {
    ...
    function generateLessonPatterns() {
        ...
        // 🔹 3–5 letter real-ish words (home row only)
        const words3 = [
            "sad", "dad", "lad", "ask", "add", "all", "las", "als", "fad", "daf",
            "sak", "jal", "faj", "kal", "lak", "ask", "ads", "als", "dak", "sak"
        ];

        const words4 = [
            "fall", "salt", "mask", "lass", "dada", "sala", "flak", "alad",
            "kaal", "fask", "dalk", "sald", "lads", "safa", "alak", "fala",
            "asld", "adsl", "kals", "sala"
        ];

        const words5 = [
            "salad", "flask", "salsa", "falls", "dadal", "askal", "ladak",
            "alsak", "falsa", "daska", "kalsd", "safal", "alkad", "slask",
            "alask", "falds", "dalsk", "sakal", "ladka", "falsa"
        ];
        ...
        // 🔹 Level 1: Real words (3–5 letters)
        for (let i = 0; i < allWords.length; i += 4) {
            practiceTexts.push(allWords.slice(i, i + 4).join(" "));
        }

        // 🔹 Level 2: Random words (harder)
        for (let i = 0; i < 10; i++) {
            let line = [];
            for (let j = 0; j < 4; j++) {
        ...
        // 🔹 Level 3: Pure random letters (ADVANCED chaos)
        for (let i = 0; i < 8; i++) {
            practiceTexts.push(
                `${randomLetters(5)} ${randomLetters(5)} ${randomLetters(5)}`
            );
        }

        // 🔹 Level 4: Mixed words + random letters (PRO)
        for (let i = 0; i < 10; i++) {
            practiceTexts.push(
