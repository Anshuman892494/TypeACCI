...
export default function TopRow() {
    ...
    function generateLessonPatterns() {
        ...
        // 🔹 Single key repetition (10 letters each)
        topRowKeys.forEach(key => {
            for (let i = 0; i < 1; i++) { // 2 lines per key
                practiceTexts.push(Array(10).fill(key).join(" "));
            }
        });

        // 🔹 Two-key alternation (10 letters each)
        for (let i = 0; i < topRowKeys.length; i += 2) {
            if (i + 1 < topRowKeys.length) {
        ...
        // 🔹 Cross-hand alternation (10 letters)
        for (let i = 0; i < topRowKeys.length / 2; i++) {
            const leftKey = topRowKeys[i];
            const rightKey = topRowKeys[i + 4]; // cross-hand
            let line = [];
            for (let k = 0; k < 5; k++) line.push(leftKey, rightKey);
            practiceTexts.push(line.join(" "));
        }

        // 🔹 Mixed 10-letter patterns (sliding window)
        const mixed = topRowKeys.concat(topRowKeys); // to wrap around
        for (let i = 0; i < topRowKeys.length; i++) {
